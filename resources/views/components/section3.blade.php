...
<section id="booking" class="py-24 bg-slate-50 font-sans min-h-screen">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div id="lux-toast"
            ...
            <div
                class="bg-white border-[3px] border-slate-900 p-6 shadow-[10px_10px_0px_0px_rgba(59,130,246,1)] flex items-center gap-4 min-w-[320px]">
                <div id="toast-icon-container"
                    class="flex-shrink-0 w-10 h-10 bg-slate-900 flex items-center justify-center">
                    <span class="h-3 w-3 rounded-full bg-blue-500 animate-ping"></span>
                </div>
                <div>
                    <p id="toast-title" class="text-[10px] font-[1000] uppercase tracking-[0.2em] text-blue-600">System
                        Message</p>
                    <p id="toast-message"
                        class="text-sm font-bold text-slate-900 uppercase tracking-tight leading-tight"></p>
                </div>
            </div>
        </div>

        <div
            class="grid grid-cols-1 lg:grid-cols-12 gap-0 border-2 border-slate-900 bg-white shadow-[20px_20px_0px_0px_rgba(15,23,42,1)]">

            <div
                class="lg:col-span-4 bg-slate-900 p-10 text-white border-b-2 lg:border-b-0 lg:border-r-2 border-slate-900">
            ...
            <div class="lg:col-span-8 p-8 md:p-12 bg-white text-slate-900">
                ...
                <h4
                    class="font-black uppercase text-xs tracking-widest text-slate-900 mb-8 underline decoration-blue-600 decoration-4 underline-offset-8">
                    Schedule Pickup Window</h4>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
                    <div>
                        <div class="flex justify-between items-center mb-6">
                            <span id="current-month" class="font-black text-sm uppercase text-slate-900">Month
                                Year</span>
                            <div class="flex gap-1">
                                <button type="button" onclick="changeMonth(-1)"
                                    class="p-1 border border-slate-200 hover:bg-slate-100">
                        ...
                        <div
                            class="grid grid-cols-7 gap-1 text-center text-[9px] font-black uppercase text-slate-400 mb-4">
                            <div>Mo</div>
                            <div>Tu</div>
                            <div>We</div>
                            <div>Th</div>
                    ...
                    <div>
                        <p class="text-[9px] font-black uppercase text-slate-400 mb-4 tracking-widest">Available Slots
                            (9AM - 9PM)</p>
                        <div id="time-slots-container"
                            class="grid grid-cols-1 gap-2 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar border-t pt-4">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                    <div class="relative">
                        <label
                            class="absolute -top-3 left-4 bg-white px-2 text-[10px] font-[1000] uppercase tracking-[0.2em] text-slate-900 z-10">Full
                            Name</label>
                        <input id="customer-name" type="text" placeholder="Full Name"
                            class="w-full border-[3px] border-slate-900 px-5 py-4 font-bold text-sm bg-white shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] focus:outline-none focus:bg-blue-50 transition-all">
                    </div>
                    <div class="relative">
                        <label
                            class="absolute -top-3 left-4 bg-white px-2 text-[10px] font-[1000] uppercase tracking-[0.2em] text-slate-900 z-10">Email
                            Address</label>
                    ...
                    <div class="relative">
                        <label
                            class="absolute -top-3 left-4 bg-white px-2 text-[10px] font-[1000] uppercase tracking-[0.2em] text-slate-900 z-10">WhatsApp</label>
                        <input id="customer-phone" type="text" placeholder="Phone"
                            class="w-full border-[3px] border-slate-900 px-5 py-4 font-bold text-sm bg-white shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] focus:outline-none focus:bg-blue-50 transition-all">
                    </div>
                </div>
                <div class="mt-10">
    ...
    <!-- Title -->
    <h2 class="text-2xl font-[1000] uppercase tracking-tight mb-2 text-slate-900">
        Choose Your Payment Method
    </h2>

    <p class="text-slate-500 text-xs mb-8 font-bold uppercase tracking-wide">
        Pay online or reserve quickly via WhatsApp
    </p>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <!-- WhatsApp Option -->
        <div class="border-2 border-slate-900 bg-white shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] p-6">
            ...
            <button onclick="sendToWhatsApp()"
                ...
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M20.52 3.48A11.82 11.82 0 0012.01 0C5.39 0 .02 5.37.02 11.99c0 2.12.55 4.18 1.6 6.01L0 24l6.2-1.62a11.94 11.94 0 005.81 1.48h.01c6.62 0 11.99-5.37 11.99-11.99 0-3.2-1.25-6.21-3.49-8.39z"/>
                </svg>
        ...
        <!-- PayPal Option -->
        <div class="border-2 border-slate-900 bg-slate-50 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] p-6 text-center">

            <p class="text-blue-600 font-black uppercase tracking-[0.3em] text-[10px] mb-4">
                Secure Payment
            </p>

            <h3 class="text-xl font-[1000] uppercase tracking-tight mb-6 text-slate-900">
                Pay Online with <span class="text-blue-600 italic">PayPal</span>
            </h3>
            ...
            <div id="paypal-button-container" class="max-w-[300px] mx-auto"></div>
...
Thank you!`;
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let btnClass = isPast ? "text-slate-200 pointer-events-none" :
                "border border-slate-100 hover:border-slate-900 font-black text-slate-900";
            if (isSelected) btnClass =
                "bg-blue-600 text-white border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]";
            grid.innerHTML +=
                `<button type="button" onclick="selectDate('${dateStr}')" class="aspect-square flex items-center justify-center text-xs transition-all ${btnClass}">${i}</button>`;
        }
    }
    ...
    function renderTimeSlots() {
        ...
        for (let h = 9; h < 21; h++) {
            ...
            container.innerHTML +=
                `<label class="block cursor-pointer"><input type="radio" name="pickup_slot" value="${time}" class="peer hidden" ${h === 9 ? 'checked' : ''} onchange="selectedTime='${time}'"><div class="p-3 border-2 border-slate-100 font-black text-slate-900 text-[11px] text-center uppercase peer-checked:border-slate-900 peer-checked:bg-slate-50 transition-all hover:border-slate-400">${time}:00 - ${(h+1).toString().padStart(2,'0')}:00</div></label>`;
        }
    }
    ...
    function showLuxToast(message, isError = false) {
        ...
        if (isError) {
            ...
            title.className = "text-red-600 font-black uppercase text-[10px]";
            container.style.boxShadow = "10px 10px 0px 0px red";
        } else {
            title.innerText = "Success";
            title.className = "text-blue-600 font-black uppercase text-[10px]";
        ...
        createOrder: function(data, actions) {
            ...
            return actions.order.create({
                purchase_units: [{
                    ...
                    description: selectedPack.getAttribute('data-name')
                }]
            });
        },
